                        ...
                        uint32_t               size_in_words)
{
    ...
    flashwriting_block_write(p_dst, p_src, size_in_words);
}
...
static void flash_page_erase(uint32_t page_number)
{
    ...
    flashwriting_page_erase((uint32_t *) m_cmd_queue.cmd[page_number].storage_addr.block_id
    );
}
                           ...
                           pstorage_handle_t       * p_block_id)
{
    ...
    // BLOCK_SIZE_CHECK(p_module_param->block_size); //TODO
    // BLOCK_COUNT_CHECK(p_module_param->block_count, p_module_param->block_size); //TODO
    ...
    // p_block_id->block_id  = m_next_page_addr; //TODO this is not the right address
...
{
    VERIFY_MODULE_INITIALIZED(); //working
    NULL_PARAM_CHECK(p_dest);     //working
    MODULE_ID_RANGE_CHECK(p_dest); // working
    BLOCK_ID_RANGE_CHECK(p_dest); //working


    if ((!is_word_aligned((uint32_t *)p_dest->block_id)))
    {
        return NRF_ERROR_INVALID_ADDR; //working
    }
    ...
    {
        return NRF_ERROR_INVALID_PARAM; //untested
    }
