...
uint32_t bootloader_settings_save(bootloader_settings_t * p_settings)
{
...
uint32_t bootloader_dfu_update_process(dfu_update_status_t update_status)
{
    ...
    if (update_status.status_code == DFU_UPDATE_APP_COMPLETE)
    {
        ...
        return bootloader_settings_save(&settings);
    }
