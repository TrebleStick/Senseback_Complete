...
int main(void)
{
    ...
    //access linker file ponters
    extern char _eapp, _uicr ,  _euicr;

    //define pointer to the start of both the load and virtual memory addresses for the UICR data
    char *src = &_eapp;
    char *dst = &_uicr;

    // write the UICR data across on boot, this should be indpendant of the start address of the application
    while(dst < &_euicr){
      *dst = *src;
